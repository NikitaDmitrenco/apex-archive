...
Last updated: 2026-09-04 (Milestone 2 completed)

---

## Current milestone

Milestone 2 — Database & Data Layer (завершён, ожидает подтверждения пользователя)

## Overall progress

~20% (архитектура, скелет приложения, рабочая БД с данными и слоем доступа; реальных
страниц ещё нет — они начинаются с Milestone 3)
...
Залито: **10 команд, 20 пилотов, 11 сезонов, 12 трасс, 18 машин, 17 связей**
driver↔team↔season. Источники и методология — `lib/db/seed/DATA_SOURCES.md` (читать перед
любым изменением данных).
...
8. **Извлечение сводных таблиц Wikipedia через WebFetch** — вернуло неверные агрегаты
   (Red Bull 4 титула, Renault 0, чемпион-2024 Норрис). **Решение:** запрашивать сырые
   годовые таблицы и считать самому; спорные значения сверять отдельным поиском по
   независимым источникам. Применять этот же подход при расширении данных.
...
**Milestone 3 — Landing / Home.** Начинать только по команде пользователя.

Полностью законченная главная страница по разделу 4.1 MASTERPROMPT на реальных данных из БД:
hero, editorial-переход при скролле, featured cars / legendary drivers / historical moments /
teams / circuits, статистика архива, блок «NOW — 2026 SEASON».

Что учесть:

- Landing будет первой страницей, читающей БД → появится реальная зависимость от
  `.env.local` при сборке.
- **Блок «NOW — 2026 SEASON» проблемный:** сезон 2026 в БД без чемпионов и без standings
  (`driver_standings` пуст). Нельзя выдумать текущие позиции. Варианты: показать календарь/
  участников без таблицы, либо запросить у пользователя актуальные данные, либо взять из
  проверяемого источника. Обсудить до реализации.
- «Статистика архива» должна считать то, что реально есть в БД (число машин/пилотов/сезонов),
  и честно называться статистикой архива, а не Формулы-1 в целом.
- Не рендерить NULL как 0 — см. "Database status".
...
- **`DIRECT_URL` — это session pooler, а не прямое подключение.** Прямое доступно только по
  IPv6. См. "Failed approaches" п.6.
